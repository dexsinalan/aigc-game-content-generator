...
if option == "首页":
    ...
    **核心功能：**
    - 📝 游戏文本设计：角色描述、剧情对话、任务文本等
    - 🖼️ 游戏美术资源：场景、角色设计、道具图标等
    - 📊 游戏数据配置：角色属性、任务列表、物品数据等
    - 🌍 游戏多语言本地化：支持18种语言翻译
...
if option == "介绍文档":
    ...
    **核心功能详解：**
    ...
    - 🎮 玩家体验预测器：基于PXI (Player Experience Inventory) 框架，分析游戏玩法描述并预测玩家体验维度评分，生成雷达图可视化
    - 🏗️ 关卡原型生成器：基于混合主动式设计理念，生成ASCII地图和JSON格式关卡，支持地图编辑和背景故事生成
    - 🎲 VGDL生成器：将自然语言游戏描述转换为标准VGDL (Video Game Description Language) 代码，支持生成Pygame脚本和实时游戏原型测试
    
    **详细使用流程：**
    1. **配置API密钥**：在"API设置"页面配置您要使用的AI模型API密钥
    2. **选择功能模块**：根据您的需求选择相应的功能模块
