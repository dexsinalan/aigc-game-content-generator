...
if option == "首页":
    ...
    **核心功能概览：**
    - 📝 游戏文本设计：角色、剧情、任务等文案内容
    - 🖼️ 游戏美术资源：场景、角色、道具等视觉素材
    - 📊 游戏数据配置：属性、任务、物品等结构化数据
    - 🌍 游戏多语言本地化：支持18种语言翻译
    - 🎮 玩家体验预测器：基于PXI框架的游戏体验分析
    - 🏗️ 关卡原型生成器：混合主动式关卡设计工具
    - 🎲 VGDL生成器：自然语言转游戏逻辑描述语言
...
if option == "介绍文档":
    st.header("📖 智造-AI游戏设计生成器使用指南")
    st.write("""
    这是一个专为游戏开发设计的AI辅助工具，使用先进的大模型技术为游戏开发者提供全方位的内容生成和设计支持。
    
    **核心功能详解：**
    - 📝 **游戏文本设计**：生成游戏角色描述、剧情对话、任务文本、技能描述、物品描述、世界观设定、NPC对话等各种游戏文案内容，支持多种风格和类型的游戏文本生成。
    - 🖼️ **游戏美术资源**：生成游戏场景、角色设计、道具图标、UI元素、特效概念图、环境概念艺术等视觉素材，支持不同艺术风格和美术方向的生成。
    - 📊 **游戏数据配置**：生成角色属性表、任务列表、物品数据、技能参数、敌人AI行为、掉落表、商店物品等结构化数据，支持JSON、XLSX和Mindmap等多种格式导出。
    - 🌍 **游戏多语言本地化**：将游戏文本翻译为18种语言版本，包括英语、日语、韩语、法语、德语、西班牙语等，支持游戏国际化部署。
    - 🎮 **玩家体验预测器**：基于PXI (Player Experience Inventory) 框架，分析游戏玩法描述并预测玩家体验维度评分，生成雷达图可视化，帮助开发者优化游戏体验。
    - 🏗️ **关卡原型生成器**：基于混合主动式设计理念，生成ASCII地图和JSON格式关卡，支持地图编辑和背景故事生成，帮助开发者快速构建关卡原型。
    - 🎲 **VGDL生成器**：将自然语言游戏描述转换为标准VGDL (Video Game Description Language) 代码，支持生成Pygame脚本，实现游戏原型的快速验证。
    
    **详细使用流程：**
    1. **配置API密钥**：在"API设置"页面配置您要使用的AI模型API密钥，不同模型需要不同的密钥配置。
    2. **选择功能模块**：根据您的需求选择相应的功能模块，每个模块针对不同类型的内容生成。
    3. **输入设计需求**：在文本输入框中详细描述您的设计需求，越详细越好，包括风格、类型、具体要求等。
    4. **选择AI模型**：从下拉菜单中选择您要使用的AI模型，不同模型在不同类型的生成任务上有各自的优势。
    5. **调整参数**：根据需要调整生成参数，如温度值、生成长度等，以获得最佳的生成效果。
    6. **生成内容**：点击生成按钮，等待AI生成内容，生成时间根据任务复杂度和模型性能有所不同。
    7. **查看和下载**：查看生成结果，根据需要下载为相应格式，如文本文件、图像文件或数据文件。
    
    **支持的AI模型：**
    - **免费API**：
      - 阿里通义千问：强大的文本和图像生成能力，适合大多数游戏开发场景
      - 智谱AI：GLM-4大模型，性能优秀，支持多语言生成
    - **付费API**：
      - 百度文心一言：文心大模型，功能全面，支持多种生成任务
      - 讯飞星火：星火大模型，支持多种任务，响应速度快
      - Claude：Anthropic的Claude模型，擅长长文本生成和复杂推理
      - ChatGPT：OpenAI的ChatGPT模型，生成质量高，创意丰富
      - DeepSeek：DeepSeek大模型，代码生成能力强
      - 硅基流动：硅基流动大模型，支持多模态生成
    
    **使用注意事项：**
    - 提示词越详细，生成结果越符合您的需求，建议提供具体的风格、场景、角色设定等信息
    - 不同模型的生成风格和能力有所不同，建议根据具体需求选择合适的模型
    - 对于复杂的生成任务，可能需要多次调整提示词以获得最佳结果
    - 生成的内容可能需要人工编辑和调整，以确保符合游戏的整体风格
    - API密钥请妥善保管，不要分享给他人，避免不必要的费用和安全风险
    - 部分模型可能有调用次数限制，请合理使用，避免过度调用导致服务中断
    - 对于图像生成，建议明确指定艺术风格、视角、光照等细节，以获得更符合预期的结果
    - 对于数据生成，建议明确数据结构、字段含义和数据范围，以确保生成的数据可直接使用
    
    **技术架构：**
    - **前端**：Streamlit（交互式Web界面），提供直观的用户交互体验
    - **后端**：Python（核心逻辑处理），实现各种生成功能和API调用
    - **数据处理**：Pandas、OpenPyXL等库，处理和导出各种格式的数据
    - **数据可视化**：Plotly（雷达图、图表等），展示玩家体验分析结果
    - **游戏原型**：Pygame，支持游戏原型的快速验证
    

    
    **最佳实践：**
    - **文本生成**：提供具体的角色背景、场景设定和风格要求，明确文本的用途和上下文
    - **图像生成**：描述清晰的场景、角色特征、艺术风格、视角和光照，指定图像的用途和尺寸
    - **数据生成**：明确数据结构、字段含义、数据范围和格式要求，提供示例数据结构
    - **多语言本地化**：确保原文清晰易懂，避免使用过于复杂的表达方式和文化特定的内容
    - **玩家体验分析**：提供详细的游戏玩法描述，包括核心机制、挑战程度和玩家目标
    - **关卡设计**：明确关卡类型、主题、难度和目标，指定关键元素和布局要求
    - **VGDL生成**：提供清晰的游戏规则、对象类型和交互逻辑，明确游戏的胜利条件和失败条件
    
    **常见问题解答：**
    - **Q: 为什么生成失败？**
      A: 检查API密钥是否正确配置，网络连接是否正常，提示词是否清晰明确。
    - **Q: 如何提高生成质量？**
      A: 使用更详细的提示词，调整temperature参数，多次尝试不同的模型和参数组合。
    - **Q: 支持哪些数据格式？**
      A: 目前支持JSON、XLSX和Mindmap格式，可根据需要选择合适的导出格式。
    - **Q: 可以添加更多AI模型吗？**
