...
# 硅基流动图像生成
def generate_image_silicon(prompt):
    """使用硅基流动生成图像"""
    api_key = os.getenv('SILICON_API_KEY')
...
# 硅基流动数据生成
def generate_data_silicon(data_prompt, data_type):
    """使用硅基流动生成数据"""
    api_key = os.getenv('SILICON_API_KEY')
    
    if not api_key:
        return None, "请配置硅基流动API密钥"
    ...
    try:
        ...
        # 调用文本生成
        result = generate_text_silicon(data_prompt)
        ...
        if data_type == "JSON":
            try:
                ...
                return data, f"generated_json_silicon.json"
...
完整生成内容：{result}"""
            ...
            try:
                ...
                if isinstance(data_list, list) and len(data_list) > 0:
                    ...
                    return df, f"generated_xlsx_silicon.xlsx"
                else:
...
完整生成内容：{result}"""
            ...
            return result, f"generated_mindmap_silicon.txt"
        else:
            return None, "不支持的数据类型"
...
def translate_text_silicon(prompt):
    """使用硅基流动翻译文本"""
    api_key = os.getenv('SILICON_API_KEY')
    ...
    try:
        result = generate_text_silicon(prompt)
        return result, "翻译成功"
    except Exception as e:
